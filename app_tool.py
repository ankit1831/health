        ...
        with st.chat_message("assistant"):
            with st.spinner("Analyzing..."):
                try:
                    ...
                    is_bailout = any(trigger in user_input.lower() for trigger in ["predict now", "stop", "enough"])
                    
                    # 1. CHECK FOR FALLBACK FIRST (Did they ignore us 3 times?)
                    if is_missing_demographics and not is_bailout:
                        if st.session_state.demo_ask_count >= 3:
                    ...
                    if is_missing_demographics and not is_bailout:
                        ...
                        api_args = {
                            "messages": [{"role": "system", "content": dynamic_system_prompt}] + st.session_state.messages[1:],
                    ...
                    # 3. THE TRIAGE NURSE (We have the data, or we forced the defaults)
                        ...
                        # Only give the Triage Tool if 5 turns have passed or bailout triggered
                        user_turns = sum(1 for m in st.session_state.messages if m["role"] == "user")
                        if user_turns >= 5 or is_bailout:
